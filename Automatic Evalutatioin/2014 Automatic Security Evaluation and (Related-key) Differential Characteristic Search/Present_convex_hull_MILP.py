...
#add the constrains selected from convex hull inequality
def add_convex_hull(i1,i2,i3,i4,o1,o2,o3,o4,f):
    
    for t in Schi:
        f.write( ("p.add_constraint("+str(t[0])+'*'+ "x["+str(i1)+"] + "+ \
                                  str(t[1])+'*'+ "x["+str(i2)+"] + "+ \
                                  str(t[2])+'*'+ "x["+str(i3)+"] + "+ \
                                  str(t[3])+'*'+ "x["+str(i4)+"] + "+ \
                                  str(t[4])+'*'+ "x["+str(o1)+"] + "+ \
...
def EqSOut(i1,i2,i3,i4,o1,o2,o3,o4,next_i,next_o,d,f):
  ...
  add_convex_hull(i1,i2,i3,i4,o1,o2,o3,o4,f)
...
def giveEquationsPresentandSolve(a,s,next,dummy,sb,O,CSOut,CSIn,next_i,next_o,rounds):
  ...
  f.write("p.write_lp(\"Present_convex_hull_lp_problem.lp\")"+"\n")
  #f.write("print \"Minimal number of S-boxes:\", solution"+'\n')
  f.close()
  execfile('Eq-Present-With-'+str(rounds)+'-Rounds-Differential.sage')

#将所有变元定义为全局变量是为了方便理解代码
exec_round=31 #block cipher exec round
...
global Schi #select convex hull inequality
Schi=[[3, 4, 4, 1, -2, 0, -2, 1, 0],
[0, -2, -2, 3, 4, 1, 4, 1, 0],
[-2, 1, 1, 3, 1, -1, 1, 2, 0],
[1, -3, -2, -2, 3, -4, 1, -3, 10],
[-1, -2, -2, -1, -1, 2, -1, 0, 6],
...
giveEquationsPresentandSolve(a,s,next,dummy,sb,O,CSOut,CSIn,next_i,next_o,exec_round)